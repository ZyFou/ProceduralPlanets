...
const {
  DEFAULT_PARAMS, STAR_DEFAULTS, GAS_DEFAULTS, PLANET_PRESETS, GAS_PRESETS, STAR_PRESETS,
} = await import(pathToFileURL(presetsPath).href);
...
const TYPE_NAME = { planet: 'terrestrial', gas: 'gas', star: 'star' };
let presetsMd = `<!-- GENERATED by scripts/gen-param-docs.mjs from src/engine/presets.js — do not edit by hand. -->
# Presets
...
`;
for (const [mode, table, title] of [['planet', PLANET_PRESETS, 'Terrestrial'], ['gas', GAS_PRESETS, 'Gas giants'], ['star', STAR_PRESETS, 'Stars']]) {
  presetsMd += `## ${title} (\`type: '${TYPE_NAME[mode]}'\`)\n\n| Preset | Label | Keys changed | Highlights |\n|---|---|---|---|\n`;
  for (const [name, preset] of Object.entries(table)) {
    const patch = preset.patch ?? {};
    const hl = HIGHLIGHT[mode].filter((k) => k in patch).map((k) => `\`${k}: ${esc(fmt(patch[k]))}\``).join(', ');
    const count = Object.keys(patch).length;
    presetsMd += `| \`${name}\` | ${preset.label} | ${count || '— (the defaults)'} | ${hl} |\n`;
  }
  presetsMd += '\n';
}

const outputs = {
  'docs/presets.md': presetsMd,
